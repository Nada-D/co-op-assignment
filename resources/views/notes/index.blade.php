            ...
            <div class="mx-auto mt-6 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-8 border-t border-gray-200 pt-6 sm:mt-8 sm:pt-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                ...
                <article class="flex flex-col items-start justify-between bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center gap-x-4 text-xs">
                        <time datetime="{{ $note->date }}" class="text-gray-500">{{ $note->date }}</time>
                        <a href="#"
                            class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100">{{ $note->category }}</a>
                    </div>
                    <div class="group relative">
                        ...
                        <p class="mt-2 line-clamp-3 text-sm leading-6 text-gray-600">{{$note->excerpt}}</p>
                    </div>
                    <div class="relative mt-4 flex items-center gap-x-4">
                        <a href="note/{{$note->slug}}/edit">
                            <button
